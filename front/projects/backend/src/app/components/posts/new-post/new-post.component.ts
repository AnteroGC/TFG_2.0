...
export class NewPostComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.apiService.getAllCategory().subscribe(cats => this.categories = cats.filter(c => c.tittle !== 'Uncategorized'));
  }
      ...
      this.uploadSub = upload$.subscribe((event) => {
        ...
        if (event.type === HttpEventType.UploadProgress) {
          ...
          if (this.uploadProgress === 100) {
            this.messageService.add({
              severity: 'info', summary: 'Success',
              detail: 'Canción subida ' + file.name , life: 20000000
            })
          }
        }
        if (event instanceof HttpResponse) {
          const response: any = event.body;
          this.mainSongPath = response.filePath;
        }
        setTimeout(()=> this.showProgress = false , 2000)
      })
  ...
  createPost(myForm: NgForm) {
     ...
     console.log('holi')
     const {tittle, category, content} = myForm.value;
