...
export class CategoryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subs.push(this.apiService.getAllCategory().subscribe(cats => this.categories = cats.filter(c => c.tittle !== 'Uncategorized')));
  }
  ...
  create(form: NgForm) {
    ...
    const {tittle, description} = form.value;

    this.apiService.createCategory(tittle, description).subscribe(res => {
  ...
  update(ev: CellEventResponse) {
    this.subs.push(this.apiService.updateCategory(ev.data.id, ev.data.title, ev.data.description).subscribe(res => {
      this.categories[ev.index].tittle = res.tittle;
